  ...
  const fetchPickupOrders = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/api/user/stockists/${user?.id}`,
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        throw new Error(
          response.data.message || "Failed to fetch history orders."
        );
      }
    } catch (error) {
      ...
      toast.error(
        error.response?.data?.message ||
          "An error occurred fetching history orders."
      );
    } finally {
      setIsLoading(false);
    }
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-center capitalize">
                      {formatTransactionType(pickupOrder?.transaction_type) ||
                        "-"}
                    </td>
