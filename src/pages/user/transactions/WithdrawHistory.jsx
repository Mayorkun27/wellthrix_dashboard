...
const WithdrawHistory = () => {
    ...
    const fetchWithdrawHistory = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get(`${API_URL}/api/user/${user?.id}/withdraw`, {
                headers: {
            ...
            toast.error(error.response?.data?.message || "An error occurred fetching withdrawal history!.");
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <table className="min-w-full">
                ...
                <tbody>
                    {isLoading ? (
                        ...
                        withdrawHistory.map((item, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <td className="p-4 capitalize">{item.transaction_type.includes("manual") ? "Manual Withdraw" : "Withdraw" || "-"}</td>
                        ...
                        <tr>
                            <td colSpan="7" className="text-center p-8">No withdraw history found.</td>
                        </tr>
                    )}
                </tbody>
            </table>
