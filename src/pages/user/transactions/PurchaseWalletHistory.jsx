...
const PurchaseWalletHistory = () => {
    const { user, token, logout } = useUser();
    const [purchaseWalletHistory, setPurchaseWalletHistory] = useState([]);
    const [isLoading, setIsLoading] = useState(true);
    const [currentPage, setCurrentPage] = useState(1);
    const [lastPage, setLastPage] = useState(1);
    const [perPage, setPerPage] = useState(5);

    const fetchpurchaseWalletHistory = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get(`${API_URL}/api/users/${user?.id}/fund-purchased-wallets`, {
                headers: {
        ...
        <div className="shadow-sm rounded bg-white overflow-x-auto">
            <table className="min-w-full">
                ...
                <tbody>
                    ...
                    ) : purchaseWalletHistory.length > 0 ? (
                        purchaseWalletHistory.map((item, index) => {
            ...
            {!isLoading && purchaseWalletHistory.length > 0 && (
