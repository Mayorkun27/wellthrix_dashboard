...
const Digital = () => {
  const [selectedDigitalProduct, setSelectedDigitalProduct] =
    useState("airtime");
  ...
  const handleTransactionSubmit = async () => {
    const toastId = toast.loading(
      `Processing ${formData.serviceID} purchase...`
    );
    if (isPinComplete) {
      setIsLoading(true);
      ...
      try {
        ...
        if (response.status === 200) {
          toast.success(
            response.data.message ||
              `${formData.serviceID} Purchase successful`,
            { id: toastId }
          );
          setTimeout(() => {
        ...
        toast.error(
          error.response.data.message ||
            "An error occured initiating transaction",
          { id: toastId }
        );
      } finally {
    ...
    <div className="space-y-6 pb-2">
      <div className="flex gap-4 overflow-x-scroll styled-scrollbar pb-3">
        {digitalProducts.map((digitalProduct, index) => (
          <div
            key={index}
            className="lg:w-[calc(100%/3)] md:min-w-2/5 lg:min-w-max min-w-10/12"
          >
            <button
              onClick={() => setSelectedDigitalProduct(digitalProduct.type)}
              className={`shadow-md rounded-xl p-3 overflow-hidden w-full cursor-pointer focus:bg-pryClr space-y-2 ${
                selectedDigitalProduct === digitalProduct.type
                  ? "bg-pryClr"
                  : "bg-white"
              } transition-all duration-300`}
            >
              <div
                className={`w-full h-[100px] rounded-lg flex items-center justify-center ${
                  selectedDigitalProduct === digitalProduct.type
                    ? "bg-white"
                    : "bg-pryClr/15"
                }`}
              >
                <img
                  src={digitalProduct.image}
                  alt={digitalProduct.name + " image"}
