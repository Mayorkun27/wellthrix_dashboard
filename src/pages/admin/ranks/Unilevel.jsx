...
const Unilevel = () => {
  ...
  return (
    <div className="space-y-4">
          ...
          <tbody>
            {currentRows.map((row, idx) => (
              <tr key={idx} className="border-t border-gray-200 hover:bg-gray-50">
                <td className="p-5 text-sm text-gray-800 align-middle">{row.name}</td>
                <td className="p-5 text-sm text-gray-800 align-middle">{row.rank}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* Use the same props shape as your AirtimeRecharges */}
