...
const ManualWithdraw = () => {
   const { token, logout } = useUser();
    ...
    const handleDatePayout = async (date, data, pinCode) => {
        ...
        try {
            const response = await axios.post(`${API_URL}/api/withdraw/process-weekly`, {
                pin: pinCode,
    ...
    return (
        <div className="space-y-6">
            {
                isLoading ? (
                    <div className="w-18 h-18 border-4 border-pryClr border-t-transparent mx-auto m-8 rounded-full animate-spin"></div>
                ) : (
                    Object.entries(manualWithdraws).map(([date, data], index) => (
                        <div key={index} className="shadow-sm rounded bg-white">
                            <div
                                className={`flex md:flex-row flex-col gap-4 md:items-center items-start justify-between w-full p-5 font-medium text-left text-gray-800 transition-colors border border-pryClr/20 rounded-[inherit] cursor-pointer ${activeAccordion === index ? 'bg-white/60' : 'bg-white/90 hover:bg-white/80'
                            }`}
                            >
                                <div className="flex items-center justify-between md:w-max w-full">
                                    ...
                                    {date === today && (
                                        <button
                                            ...
                                            className="md:hidden block bg-pryClr h-[40px] px-4 rounded-md cursor-pointer text-secClr disabled:cursor-not-allowed disabled:opacity-50"
                                        >
                                ...
                                <div className="flex items-center gap-4">
                                    {date === today && (
                                        <button
                                            ...
                                            className="md:block hidden bg-pryClr h-[40px] px-4 rounded-md cursor-pointer text-secClr disabled:cursor-not-allowed disabled:opacity-50"
                                        >
                                            {isPaying ? 'Paying...' : 'Pay All'}
                                        </button>
                                    )}
            ...
            {showPinModal && (
                ...
                <div className="flex flex-col items-center gap-6 text-center">
                    ...
                    <div className="flex gap-3">
                        {[0, 1, 2, 3].map((index) => (
                            <input
                                ...
                                className="w-12 h-12 md:w-14 md:h-14 bg-gray-200 rounded-lg text-center text-xl font-bold focus:outline-none focus:ring-2 focus:ring-pryClr border border-gray-300"
                                onKeyDown={(e) => {
                    ...
                    <button
                        ...
                        className="w-full py-3 bg-pryClr text-white rounded-lg disabled:opacity-50"
                    >
