...
const CartCards = ({ cartItem }) => {
    return (
        <div className='flex items-center justify-between py-2 md:px-6 px-4 border-b border-black/20 last:bordeer-b-0'>
            ...
            <div className="flex items-center gap-3 md:w-[40%] w-3/4 md:ms-0 ms-3">
                <div className="bg-pryClr/15 w-[80px] h-[70px] rounded-lg">
                    <img 
                        src={cartItem.image} 
                ...
                <div className="flex flex-col">
                    <h4 className='w-full leading-5 text-pryClr font-bold'>{cartItem.name}</h4>
                    <p className='md:hidden inline-flex'>{formatterUtility(Number(cartItem.price)*Number(cartItem.quantity))}</p>
                </div>
            </div>
            <div className="md:flex hidden items-center border rounded-lg overflow-hidden">
                <button 
                    onClick={() => handleDecrement(cartItem)}
                    className='w-8 h-8 flex items-center justify-center cursor-pointer'
                >
