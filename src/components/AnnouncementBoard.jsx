...
const AnnouncementBoard = ({ refresh }) => {
    ...
    const fetchAnnouncements = useCallback(async () => {
        ...
        try {
            ...
            if (response.status === 200 && response.data.status === "success") {
                ...
                const readIds = getReadAnnouncementIds();
                const latestUnread = [...fetchedAnnouncements]
                    .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
                    .find(announcement => !readIds.has(String(announcement.id)));

                if (latestUnread && user?.role !== "admin") {
                    setSelectedAnnouncementAttachment(latestUnread);
                }
            }
        } catch (error) {
    ...
    }, [token, refresh, logout, getReadAnnouncementIds, user?.role]);
    ...
    const handleViewDetails = () => {
        setPopupAnnouncement(selectedAnnouncementAttachment);
        setSelectedAnnouncementAttachment(null);
    };
    ...
    }, [token, logout, getReadAnnouncementIds]);
    ...
    return (
        <>
            ...
            {/* First popup: Image only */}
            {selectedAnnouncementAttachment && (
                <Modal onClose={() => setSelectedAnnouncementAttachment(null)}>
                    <div className="w-full text-center">
                        <img src={`${IMAGE_BASE_URL}/${selectedAnnouncementAttachment?.image}`} alt="Announcement" className="lg:w-1/2 md:w-3/4 mx-auto rounded-lg mb-4" />
                        <button
                            onClick={handleViewDetails}
                            className="underline text-lg cursor-pointer text-pryClr hover:text-pryClrDark"
                        >
                            View Details
                        </button>
                    </div>
            ...
            {/* Second popup: Full details and mark as read */}
            {popupAnnouncement && (
            ...
            {/* Modal for viewing details from the list */}
                        ...
                        {user?.role === "admin" && (
                            <div className="text-center mt-6">
                                <button
                                    ...
                                    className="bg-red-500 md:w-1/2 w-full h-[50px] text-white rounded-lg cursor-pointer mx-auto disabled:opacity-50 disabled:cursor-not-allowed"
                                >
